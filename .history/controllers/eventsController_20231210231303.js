...
//gets the parameter using a function to reduce clutter
...
module.exports = {
  ...
  //action to create a form and using flash. only authenticated users can do this
  create: (req, res, next) => {
  ...
  //editing a specifc event only allows admin and the one who created the event to edit
  edit: (req, res, next) => {
    if (!res.locals.loggedIn) {
  ...
  //deleting an event by finding it and removing it. only allows admin to do this
  delete: (req, res, next) => {
    if (!res.locals.loggedIn) {
  ...
  //respond kson which returns the json representation of the data
  respondJSON: (req, res) => {
    res.json({
  ...
  //filters the events to show which ones are attended and which ones aren't in the modal
  filterUserEvents: (req, res, next) => {
    ...
    if (currentUser) {
      ...
      let mappedEvents = res.locals.events.map((event) => {
        ...
        let userJoined = event.attendees.includes(currentUser.id);
        // Add a 'joined' property to the course object indicating whether the user rsvped for the event
