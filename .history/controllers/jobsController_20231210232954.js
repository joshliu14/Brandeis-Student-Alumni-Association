...
module.exports = {
  ...
  //create new job. only authenticated users can do this.
  create: (req, res, next) => {
  ...
  //find id of certain job to edit. only the one who created this and the admin can edit
  edit: (req, res, next) => {
    if (!res.locals.loggedIn) {
  ...
  //edit and update a job from the form submition
  update: (req, res, next) => {
    let jobId = req.params.id,
  ...
  //find id of job and delete it. only the admin can do this
  delete: (req, res, next) => {
    if (!res.locals.loggedIn) {
